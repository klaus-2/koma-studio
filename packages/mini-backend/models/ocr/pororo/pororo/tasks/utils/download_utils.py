...
# mini-backend path:
# models/ocr/pororo/pororo/tasks/utils/download_utils.py
# We want the local model directory at models/ocr/pororo/
models_base_dir = os.path.abspath(
    os.path.join(current_file_dir, '..', '..', '..')
)
MODEL_ROOT_ENV = "KOMA_MODELS_ROOT"
ALLOW_DOWNLOAD_ENV = "KOMA_PORORO_ALLOW_DOWNLOAD"
MANAGED_SUBDIR = "pororo"
...
def get_save_dir(save_dir: str = None) -> str:
    ...
    managed_root = os.getenv(MODEL_ROOT_ENV, "").strip()
    if managed_root:
        save_dir = os.path.join(
            os.path.abspath(os.path.expanduser(managed_root)),
            MANAGED_SUBDIR,
        )
        os.makedirs(save_dir, exist_ok=True)
        return save_dir
...
def download_or_load_transformer(info: DownloadInfo) -> TransformerInfo:
    ...
    """
    config = CONFIGS[os.path.basename(info.n_model)]
    ...
    model_type_dir = os.path.dirname(model_path) or "."
    if not os.path.exists(model_path):
    ...
    if src_dict_in:
        ...
        dict_type_dir = os.path.dirname(src_dict_path) or "."
...
def download_from_url(
    ...
    """
    allow_download = os.getenv(ALLOW_DOWNLOAD_ENV, "").strip() == "1"
    if not allow_download:
        raise RuntimeError(
            "Pororo OCR models are not installed in the managed local directory. "
            "Open the Model Manager and install Pororo OCR before using it.",
        )

    # Get default type dir path
    type_dir = os.path.dirname(model_path) or "."
