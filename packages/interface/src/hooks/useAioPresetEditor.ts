...
import { AIO_PRESET_STAGE_KEYS, AIO_STAGE_LABELS } from '../constants/dashboard.constants';
import {
...
import { createEmptyAioPresetEditorDraft, normalizePresetLanguage } from '../utils/dashboard.utils';
import type { AioLanguageModelPreset, AioModelPresetStateV2, AioStageSelection } from '../types/aioModelPresets';
import type { AioPresetEditorDraft } from '../types/dashboard.types';
import type { AioLanguageOption, AioStageOption } from '../models/aioStageCatalog';

interface UseAioPresetEditorArgs {
  aioSrcLang: string;
  aioLanguageSourceOptions: AioLanguageOption[];
  aioPresetState: AioModelPresetStateV2;
  setAioPresetState: React.Dispatch<React.SetStateAction<AioModelPresetStateV2>>;
  aioStageSelectionRef: React.MutableRefObject<AioStageSelection>;
...
export function useAioPresetEditor({
  ...
  const applyPresetSelection = useCallback((preset: AioLanguageModelPreset) => {
    const currentSelection = aioStageSelectionRef.current;
  ...
  }, [aioStageSelectionRef, presetStageAvailability, setAioStageSelection, setStatusMessage, t]);
  ...
  const openCreatePresetEditor = useCallback(() => {
    setAioPresetEditorDraft(
      createEmptyAioPresetEditorDraft(
        sanitizePresetEditorStageModels({ ...aioStageSelectionRef.current }),
      ),
    );
    setAioPresetEditorOpen(true);
  }, [aioStageSelectionRef, sanitizePresetEditorStageModels]);
  ...
  const handleSaveCurrentSelectionAsPreset = useCallback(() => {
    ...
    const stageModels = sanitizePresetEditorStageModels({ ...aioStageSelectionRef.current });
  ...
  }, [aioPresetState, aioStageSelectionRef, normalizedAioSourceLanguage, sanitizePresetEditorStageModels, setAioPresetState, setStatusMessage, sourceLanguageLabelByCode, t]);
