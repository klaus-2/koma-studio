...
const hardenIndexHtml = () => {
  ...
  //  - app://local must appear in connect-src for fetch() to the custom protocol.
  //  - worker-src allows web workers to be loaded from same origin.
  if (!html.includes("Content-Security-Policy")) {
    const cspMeta = `<meta http-equiv="Content-Security-Policy" content="default-src 'self' app://local; base-uri 'self'; object-src 'none'; script-src 'self' app://local https://challenges.cloudflare.com; frame-src 'self' https://challenges.cloudflare.com; worker-src 'self' blob: https://challenges.cloudflare.com; style-src 'self' 'unsafe-inline' app://local; img-src 'self' data: blob: https:; connect-src 'self' app://local https: ws://127.0.0.1:* http://127.0.0.1:*; font-src 'self' data: https://fonts.gstatic.com;">`;
